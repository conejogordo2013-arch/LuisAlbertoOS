...
- Desktop loop en `kernel/desktop.asm` con entrada PS/2 + composición final.
...
- Zero-fill de `.bss` (`p_memsz - p_filesz`).
- Obtiene `e_entry` y ejecuta con stack dedicado.
- Si la app retorna sin `exit`, se invoca syscall 6 (`exit_process`).
...
- `12` get_key
- `6` exit_process
- Implementadas en `kernel/interrupts.lasys` (`isr_syscall80`).
...
- `apps/hello.elf.asm`:
  ...
  - Dibuja pixel.
  - Invoca `exit`.
- Launcher integrado en `kernel/applications.asm` leyendo sectores ELF y llamando `elf_run_image`.
